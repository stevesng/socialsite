...
/**
 * For persisting a TokenInfo.
 * Part of SocialSite OAuth Consumer implementation.
 */
@Entity
@Table(name="ss_oauthaccessor")
public class OAuthAccessorRecord implements Serializable {
    ...
    public OAuthAccessorRecord(BasicOAuthStoreTokenIndex index, TokenInfo info) {
        this.id = index.hashCode();
        update(info);
    }

    public TokenInfo getTokenInfo() {
        return new TokenInfo(accessToken, tokenSecret, sessionHandle, expireMillis);
    }

    public void update(TokenInfo info) {
        this.accessToken   = info.getAccessToken();
        this.tokenSecret   = info.getTokenSecret();
        this.sessionHandle = info.getSessionHandle();
        this.expireMillis  = info.getTokenExpireMillis();
