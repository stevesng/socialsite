 ...
 * SocialSite Groups.
 */
public class GroupImageServlet extends ImageServlet {

    private static final long serialVersionUID = 0L;
    private static Log log = LogFactory.getLog(GroupImageServlet.class);

    /**
     * Public constructor.
     */
    public GroupImageServlet() {
        super();
    }

    protected Result getResult(String itemName) throws IOException, ServletException {

        Group group;

        try {
            GroupManager mgr = Factory.getSocialSite().getGroupManager();
            group = mgr.getGroupByHandle(itemName);
        ...
        return new Result(group.getUpdated(), group.getImageType(), (byte[]) (group.getImage()));
    }
}
