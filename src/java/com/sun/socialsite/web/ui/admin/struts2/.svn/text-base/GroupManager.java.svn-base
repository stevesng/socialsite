...
/**
 * Group Manager Form Action.
 */
public class GroupManager extends CustomizedActionSupport {
    ...
    public GroupManager() {
        this.setPageTitle("Group.pageTitle");
    ...
    public List<Group> getGroups() throws Exception {
        com.sun.socialsite.business.GroupManager gpMgr =
            Factory.getSocialSite().getGroupManager();
        if (pattern != null && pattern.trim().length() > 0) {
            groups = gpMgr.searchGroups(0, -1, pattern);
        } else {
            groups = gpMgr.getGroups(0, -1);
        }
        return groups;
    }
