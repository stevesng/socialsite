...
function getData() {
  ...
  req.add(req.newFetchPeopleRequest(spec, opt_params), 'viewerFriends');

  req.add(socialsite.newFetchFriendsGroupsRequest(opensocial.IdSpec.PersonId.VIEWER), 'friendsGroups');
...
function onLoadFriends(dataResponse) {
  ...
  var numItems = viewerFriends.size();
  var item = 0;
...
function filterGroup() {
   ...
   if (filterValue != "all" && filterIndex != 0) {
       ...
       groupSpec.setField(opensocial.IdSpec.Field.USER_ID, opensocial.IdSpec.PersonId.VIEWER);
...
function composeMail(toUserId) {
   ...
   var messageUrl = location.href.replace("viewer_friends.xml", replacement);
...
function composeGroupInvite(toUserId) {
   ...
   var groupInviteUrl = location.href.replace("viewer_friends.xml", replacement);
